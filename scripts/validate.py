...
from gpux.utils.helpers import check_dependencies, get_system_info
...
def system() -> None:
    ...
    if missing_critical:
        console.print(
            f"\n[red]❌ Missing critical dependencies: "
            f"{', '.join(missing_critical)}[/red]"
        )
        raise typer.Exit(1)
...
def providers() -> None:
    ...
    try:
        ...
        for provider_name in manager.get_available_providers():
            # Convert string to ExecutionProvider enum
            try:
                provider = ExecutionProvider(provider_name)
        ...
        if available_providers:
            console.print(
                f"\n[green]✅ Found {len(available_providers)} "
                f"available providers[/green]"
            )
        else:
            console.print(
                "\n[yellow]⚠️  No GPU providers available, "
                "CPU fallback will be used[/yellow]"
            )
...
def config(
    config_file: str = typer.Argument(
        "gpux.yml", help="Configuration file to validate"
    ),
) -> None:
...
def examples() -> None:
    ...
    if all_valid:
        ...
        console.print(
            "\n[yellow]⚠️  Some examples have issues "
            "(missing model files are expected)[/yellow]"
        )
...
def validate_all() -> None:
    ...
    try:
        ...
        console.print(
            "[bold green]🎉 All validation checks completed successfully![/bold green]"
        )
