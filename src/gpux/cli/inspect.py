...
def inspect_command(
    ...
    try:
        if model_file:
            ...
            _inspect_model_file(Path(model_file), json_output)
        elif model_name:
            # Inspect model by name
            _inspect_model_by_name(model_name, config_file, json_output)
        else:
            # Show runtime information
            _inspect_runtime(json_output)
...
def _inspect_model_by_name(model_name: str, config_file: str, json_output: bool) -> None:
...
def _inspect_runtime(json_output: bool) -> None:
    ...
    if json_output:
        ...
        for provider in provider_manager._provider_priority:
        ...
        _display_runtime_info(provider_manager, available_providers)
...
def _display_config_info(config) -> None:
...
def _display_model_info(model_info) -> None:
...
def _display_runtime_info(provider_manager: ProviderManager, available_providers: list) -> None:
    """Display runtime information."""
    
    # Available providers
    providers_table = Table(title="Available Execution Providers", show_header=True, header_style="bold magenta")
    ...
    # Provider priority
    priority_table = Table(title="Provider Priority", show_header=True, header_style="bold green")
    ...
    for i, provider in enumerate(provider_manager._provider_priority, 1):
        info = provider_manager.get_provider_info(provider)
